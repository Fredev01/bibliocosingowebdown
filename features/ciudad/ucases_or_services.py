...
from features.ciudad.models import Ciudad, CiudadForm
...
class CiudadCU():
    def get_all(self, param_limit, search_value):
        ...
        sqlParams = {}
        if search_value:
            cond += prepParam(sqlParams, ' ', 'ciudad', 'like', search_value, ' ')
        if cond:
            cond= f"WHERE {cond}"
        # Obtener el total de registros a retornar
        sql_query = f"select count(1) From ciudad {cond}"
    ...
    def save(self, data : CiudadForm):
        if (data.id.data == None or data.id.data == ''  ) :
            objList = Ciudad.query.filter(Ciudad.nombre==data.nombre.data).first() or []
            if objList:
                return {"obj": None}
        else:
            objValid = Ciudad.query.filter(Ciudad.nombre==data.nombre.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Ciudad.query.filter(Ciudad.id == data.id.data).all()
        obj : Ciudad = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Ciudad()
    ...
    def delete(self, id : int):
        obj = Ciudad.query.filter(Ciudad.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
