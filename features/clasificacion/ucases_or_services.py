...
from features.clasificacion.models import Clasificacion, ClasificacionForm
from features.core.bd import db, execute_query


class ClasificacionCU():
    def get_all(self, param_limit, search_value):
        ...
        sql_query = f"select count(1) From clasificacion {cond}"
    ...
    def save(self, data : ClasificacionForm):
        if (data.id.data == None or data.id.data == ''):
            objList = Clasificacion.query.filter(Clasificacion.nombre==data.nombre.data).first() or []
            if objList:
                return {"obj": None}
        else:
            objValid = Clasificacion.query.filter(Clasificacion.nombre==data.nombre.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Clasificacion.query.filter(Clasificacion.id == data.id.data).all()
        obj : Clasificacion = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Clasificacion()
    ...
    def delete(self, id : int):
        obj = Clasificacion.query.filter(Clasificacion.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
    ...
    def get_combo(self, data : dict):
        ...
        sql_query = "select id, nombre text from clasificacion e " + cond
        registros = execute_query( sql_query, sqlParams)
