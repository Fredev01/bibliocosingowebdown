...
from features.escuela.models import Escuela, EscuelaForm
...
class EscuelaCU():
    def get_all(self, param_limit, search_value):
        ...
        sql_query = f"select count(1) From escuela {cond}"
        registros = execute_query( sql_query, sqlParams)
        total=registros[0][0]

        # Obtener los registros a retornar
        sql_query = f"""
            select e.id, e.nombre, en.id escuelanivel_id, en.nombre from escuela e
            LEFT JOIN escuelanivel en ON en.id = e.nivel_id
            {cond} {param_limit}
            """
        registros = execute_query( sql_query, sqlParams)
    ...
    def get_combo(self, data : dict):
        ...
        sql_query = "select id, nombre text from escuela e " + cond
        registros = execute_query( sql_query, sqlParams)

        return registros

        
    def save(self, data : EscuelaForm):
        if (data.id.data == '' or data.id.data == None):
            objList = Escuela.query.filter(Escuela.nombre==data.nombre.data).first() or []
            if objList:
                return {"obj": None}
            if data.nivel_id.data == '-1':
                return {"obj": "nivel_vacio"}
        else:
            objValid = Escuela.query.filter(Escuela.nombre==data.nombre.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Escuela.query.filter(Escuela.id == data.id.data).all()
        obj : Escuela = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Escuela()
        obj.nombre= data.nombre.data
        obj.nivel_id= data.nivel_id.data
    ...
    def delete(self, id : int):
        obj = Escuela.query.filter(Escuela.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
    ...
    def generar(self):
        ...
        # Obtener los registros a retornar
        sql_query = f"""
            SELECT e.id, e.nombre, en.nombre FROM escuela e
            LEFT JOIN escuelanivel en ON en.id = e.nivel_id
            {cond} 
        """
    ...
    def get_escuela(self, nombre, nivel_id):
        cond = ""
        sqlParams = {}
        cond += prepParam(sqlParams, '( ', 'nombre', '=', nombre, ' and ')
        cond += prepParam(sqlParams, ' ', 'nivel_id', '=', nivel_id, ' )')
        ...
        # Obtener los registros a retornar
        sql_query = f"""
            select nombre from escuela {cond}
            """
        registros = execute_query( sql_query, sqlParams)
        # retornar el total y los registros
        return  registros
