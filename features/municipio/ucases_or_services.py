...
from features.municipio.models import Municipio, MunicipioForm
...
class MunicipioCU():
    def get_all(self, param_limit, search_value):
        ...
        sqlParams = {}
        if search_value:
            cond += prepParam(sqlParams, ' ', 'municipio', 'like', search_value, ' ')
        if cond:
            cond= f"WHERE {cond}"
        # Obtener el total de registros a retornar
        sql_query = f"select count(1) From municipio {cond}"
    ...
    def save(self, data : MunicipioForm):
        if (data.id.data == None or data.id.data == ''  ) :
            objList = Municipio.query.filter(Municipio.nombre==data.nombre.data).first() or []
            if objList:
                return {"obj": None}
        else:
            objValid = Municipio.query.filter(Municipio.nombre==data.nombre.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Municipio.query.filter(Municipio.id == data.id.data).all()
        obj : Municipio = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Municipio()
    ...
    def delete(self, id : int):
        obj = Municipio.query.filter(Municipio.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
    ...
    def generar(self):
        ...
        pdf.ln(10)
        pdf.set_font('Times', 'B', 14.0)
        pdf.cell(page_width, 0.0, 'Registros de Municipios', align='C')
        pdf.ln(10)
