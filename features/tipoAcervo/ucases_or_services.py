...
from features.tipoAcervo.models import Tipo, TipoForm
...
class TipoCU():
    def get_all(self, param_limit, search_value):
        ...
        sqlParams = {}
        if search_value:
            cond += prepParam(sqlParams, ' ', 'tipo', 'like', search_value, ' ')
        if cond:
            cond= f"WHERE {cond}"
        # Obtener el total de registros a retornar
        sql_query = f"select count(1) From tipo {cond}"
    ...
    def save(self, data : TipoForm):
        if (data.id.data == None or data.id.data == ''  ) :
            objList = Tipo.query.filter(Tipo.nombre==data.nombre.data).first() or []
            if objList:
                return {"obj": None}
        else:
            objValid = Tipo.query.filter(Tipo.nombre==data.nombre.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Tipo.query.filter(Tipo.id == data.id.data).all()
        obj : Tipo = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Tipo()
    ...
    def delete(self, id : int):
        obj = Tipo.query.filter(Tipo.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
    ...
    def get_combo(self, data : dict):
            ...
            sql_query = "select id, nombre text from tipo e " + cond
            registros = execute_query( sql_query, sqlParams)
