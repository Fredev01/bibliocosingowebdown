...
app = Blueprint('VisitantetipoPages', __name__, url_prefix='/visitantetipo')

@app.get('/')
def get_visitantetipo():
    try:
        if not user_has_session():
            return redirect( url_for("AuthRoute.get_login", ret=url_for("VisitantetipoPages.get_visitantetipo") ) )        
        usuario_autenticado = True
        if current_user.tipo != "2":
            return render_template ("sinacceso.html", usuario_autenticado=usuario_autenticado)
        obj_form = VisitantetipoForm()
        context={
            'obj_form': obj_form,
            'usuario_autenticado' : True,
            'full_name': current_user.full_name
        }
        return render_template ("visitantetipo.html", **context)
