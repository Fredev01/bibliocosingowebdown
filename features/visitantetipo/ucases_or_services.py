...
from features.visitantetipo.models import Visitantetipo, VisitantetipoForm
from features.core.bd import db, execute_query


class VisitantetipoCU():
    def get_all(self, param_limit, search_value):
        # prepara la condicion a filtrar
        cond = ""
        sqlParams = {}
        if search_value:
            cond += prepParam(sqlParams, ' ', 'visitantetipo', 'like', search_value, ' ')
        if cond:
            cond= f"WHERE {cond}"
        # Obtener el total de registros a retornar
        sql_query = f"select count(1) From visitantetipo {cond}"
        registros = execute_query( sql_query, sqlParams)
        total=registros[0][0]

        # Obtener los registros a retornar
        sql_query = f"""
            select t.id, t.visitantetipo
            from visitantetipo t
            {cond} {param_limit}
            """
        registros = execute_query( sql_query, sqlParams)
    ...
    def get_combo(self, data : dict):
        ...
        sqlParams = {}
        if data.get('q'):
            cond += prepParam(sqlParams, '', 'e.visitantetipo', 'like', data.get('q'), ' ')
        if data.get('id'):
        ...
        sql_query = "select id, visitantetipo text from visitantetipo e " + cond
        registros = execute_query( sql_query, sqlParams)
    ...
    def save(self, data : VisitantetipoForm):
        if (data.id.data == '' or data.id.data == None):
            objList = Visitantetipo.query.filter(Visitantetipo.visitantetipo==data.visitantetipo.data).first() or []
            if objList:
                return {"obj": None}
        else:
            objValid = Visitantetipo.query.filter(Visitantetipo.visitantetipo==data.visitantetipo.data).first() or None
            if objValid :
                if objValid.id != int(data.id.data):
                    return {"obj": None}
            objList = Visitantetipo.query.filter(Visitantetipo.id == data.id.data).all()
        obj : Visitantetipo = objList[0] if len(objList)>0 else None
        if obj == None:
            obj = Visitantetipo()
        obj.visitantetipo= data.visitantetipo.data
    ...
    def delete(self, id : int):
        obj = Visitantetipo.query.filter(Visitantetipo.id == id).first()
        if obj == None:
            return {"oper": None}
        else:
